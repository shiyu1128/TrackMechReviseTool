...
该规则对所有可选元素通用，不维护 O、N 等元素各自独立的预写计划或组分表。程序从当前 `.out` 读取元素列和每个组分的组成，按组分名称中的元素段枚举标记原子的所有分配。例如选择 N 时，`N2` 生成 `NN*`、`N*2`；选择 O 时，`O2` 生成 `OO*`、`O*2`。只有名称中解析到的目标元素原子数与 `.out` 组成一致时才接受自动规则，否则在审核结果中报告 `IncompleteSpeciesRules`，由用户检查特殊组分命名。
...
“生成计划”每次都根据当前载入的 `.out` 和下拉框中的元素重新计算，不读取或复用事先写好的计划。界面显示确定型进度条；完成后可将每条候选标记反应命名保存为 `rewrite_plan_<元素>.csv`，该文件也可通过“载入计划”继续审核。用户可编辑的核心字段是 `Selected`、`ForwardProbability`、`ReverseProbability`、`RevA`、`RevN` 和 `RevE`。其余倍率、分组和附属反应标志由程序生成。
